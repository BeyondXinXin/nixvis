...
type StatsResult struct {
	Overall StatsPoint `json:"overall"` // 总体统计数据
	Charts  ChartsData `json:"charts"`  // 图表数据
}
...
// NewStatsManager 创建一个新的 StatsManager 实例
func NewStatsManager(userRepoPtr *Repository) *StatsManager {
	return &StatsManager{
		repo:        userRepoPtr,
...
// StatsByWebIDAndTimeRange 直接查询数据库获取指定时间范围和视图类型的统计数据
func (s *StatsManager) StatsByWebIDAndTimeRange(
	websiteID string, timeRange string, viewType string) (StatsResult, error) {

	timePoints, labels := util.GetTimeRangeByType(timeRange, viewType)
	result := StatsResult{
		Overall: StatsPoint{
			PV:      0,
			UV:      0,
	...
	overall, err := s.getOverallStats(
		websiteID, timePoints[0], timePoints[len(timePoints)-1])
	if err != nil {
	...
	// 获取图表数据
	err = s.getChartData(websiteID, timePoints, &result.Charts)
	if err != nil {
...
func (s *StatsManager) UpdateStats() error {
...
// 获取特定网站的统计数据文件路径
func (s *StatsManager) getStatsFilePath(websiteID string) string {
	return filepath.Join(s.dataFileDir, fmt.Sprintf("%s_stats_data.json", websiteID))
}

// loadStatsDataFromFile 从文件加载特定网站的统计数据
func (s *StatsManager) LoadStatsDataFromFile(websiteID string) (*StatsData, error) {
	statsData := &StatsData{
...
func (s *StatsManager) getOverallStats(
	websiteID string, startDate, endDate time.Time) (StatsPoint, error) {
	...
	err := s.repo.db.QueryRow(query,
		startDate.Format("2006-01-02 15:04:05"),
		endDate.Format("2006-01-02 15:04:05")).Scan(&result.PV, &result.UV, &result.Traffic)

	return result, err
}

// getChartData 获取图表数据
func (s *StatsManager) getChartData(
	websiteID string, timePoints []time.Time, chartData *ChartsData) error {

	timeOffset := timePoints[1].Sub(timePoints[0])
	for i, point := range timePoints {
		tr := &TimeRange{
			Start: point,
			End:   point.Add(timeOffset),
		}
		stats, err := s.statsByTimeRangeForWebsite(websiteID, tr)
		if err != nil {
			return fmt.Errorf("获取统计数据失败: %v", err)
		}
		chartData.Visitors[i] = stats.UV
		chartData.Pageviews[i] = stats.PV
	}

	chartData.PvMinusUv = make([]int, len(chartData.Pageviews))
	for i, pv := range chartData.Pageviews {
		uv := chartData.Visitors[i]
		chartData.PvMinusUv[i] = pv - uv
	}
...
func (s *StatsManager) createInitialStatsDataForWebsite(
...
func (s *StatsManager) saveStatsDataToFile(websiteID string, statsData *StatsData) error {
	// 确保数据目录存在
...
func (s *StatsManager) getHourlyStatsForWebsite(
...
func (s *StatsManager) statsByTimeRangeForWebsite(
